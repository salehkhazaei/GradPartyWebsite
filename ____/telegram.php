...
/*	$rows = Select ( "*", $GLOBALS['tbl_users'], "id=?", array ($_SESSION['user']) );
	if ( $row = $rows->fetch() ) */
	{
		if ( true ) //$_SESSION['user'] == 9131089 )
		{
			$tarin_length = 0;
			$crows = Select ( "COUNT(id)", $GLOBALS['tbl_bestlist'], "1", array() );
			if ( $crow = $crows->fetch() )
			{
				$tarin_length = $crow[0];
			}
...
";
				...
				$str = "(بخش عکس بچگی تا 20 فروردین فعال خواهد بود).
...
".$str;
				...
				$json_str .= 
'{
"telegram": "'.escapeJsonString($row['_telegram']).'",
